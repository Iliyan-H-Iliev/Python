...
class Controller:
    ...
    def duel(self, first_player_name: str, second_player_name: str):
        ...
        def curr_duel(smaller, greater):
            ...
            greater.stamina = max(greater.stamina - smaller.stamina / 2, 0)
            if greater.stamina == 0:
                return f"Winner: {smaller.name}"

            smaller.stamina = max(smaller.stamina - greater.stamina / 2, 0)
            if greater.stamina == 0:
                return f"Winner: {greater.name}"

            if smaller.stamina > greater.stamina:
                return f"Winner: {smaller.name}"
